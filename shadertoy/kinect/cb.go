...
func rgbCallbackGo(dev *C.freenect_device, rgbPtr *C.void, timestamp C.uint32_t) {
...
func depthCallbackGo(dev *C.freenect_device, depthPtr *C.void, timestamp C.uint32_t) {
	instanceHandle := uintptr(C.freenect_get_user(dev))
	h, ok := instances.Load(instanceHandle)
	if !ok {
		panic(fmt.Errorf("kinect: instance not found, handle=%v", instanceHandle))
